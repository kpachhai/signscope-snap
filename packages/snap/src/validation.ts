...
export async function validateTransaction(
  ...
  const banList = (await getValue('banList'))?.toString();
  const whiteList = (await getValue('whiteList'))?.toString();
  const warnOverValue = await getValue('warnOver');
  const warnOver = warnOverValue
    ? parseInt(warnOverValue.toString())
    : 999999999;
  ...
  if (
    whiteList &&
    whiteList.length > 0 &&
    (whiteList.includes(transactionTo) || whiteList.includes(transaction.to))
  ) {
    ...
    if (
      banList &&
      banList.length > 0 &&
      (banList.includes(transactionTo) || banList.includes(transaction.to))
    ) {
      blockingIssues.push(`Recipient or contract in the ban list!`);
    }

    if (transaction.value) {
      const decimalValue = parseInt(transaction.value, 16);
      const hbarValue = decimalValue / 1e18;
      if (hbarValue > warnOver) {
        warnings.push(
          `Transaction value (${hbarValue} HBAR) exceeds the warning threshold (${warnOver} HBAR).`,
        );
      }
    }
  }
 ...
 * @param transactionTo
 */
export async function isInWhitelist(transactionTo: string): Promise<boolean> {
  const whiteList = (await getValue('whiteList'))?.toString();
